...
/// \class G::MapFile
/// A class for reading and editing key=value files, supporting the creation of 
/// backup files, variable expansion, character-set encoding and logging.
/// \see G::OptionValue
/// 
class G::MapFile
{
public:
	...
	explicit MapFile( const OptionMap & map , const std::string & yes = std::string() ) ;
		///< Constructor that reads from an option value map, typically parsed out
		///< from a command-line. Unvalued 'true' options in the option value map 
		///< are loaded into this mapfile object with a value given by the 'yes' 
		///< parameter, whereas unvalued 'false' options are not loaded. Multi-valued
		///< options are loaded as a comma-separated list.
	...
	static void check( const G::Path & , bool utf8 = false ) ;
		///< Throws if the file is invalid. This is equivalent to constructing 
		///< a temporary MapFile object, but it specifically does not do 
		///< any logging.
