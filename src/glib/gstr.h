...
public:
	...
	static unsigned int replaceAll( std::string & s , const std::string & from , const std::string & to ) ;
		...
		///< occurances of sub-string 'from' with 'to'. Returns 
	...
	static void splitIntoFields( const std::string & in , StringArray & out , 
		...
		bool remove_escapes = true ) ;
			...
			///< cleared first.
			///< 
			///< If a non-null escape character is given then escaped separators do not 
			///< result in a split. If the 'remove-escapes' parameter is true then all 
			///< unescaped escapes are removed from the output; this is generally the
			///< preferred behaviour for nested escaping. If the 'remove-escapes' 
			///< parameter is false then escapes are not removed; unescaped escapes 
			///< are used to prevent splitting but they remain in the output.
	...
	static std::string splitMatchTail( const std::string & in , const std::string & s , 
		const std::string & ws = Str::ws() ) ;
			///< Splits the input string into parts and looks for a part that starts with 's'
			///< and returns the trailing substring.
	...
	static std::string::size_type ifind( const std::string & s , const std::string & key , 
