...
GNet::Location::Location( const std::string & location_string , int family ) :
	...
	m_socks_far_port(0U)
{
	m_socks = socked( location_string , m_socks_far_host , m_socks_far_port ) ;
...
bool GNet::Location::socked( const std::string & s , std::string & far_host , unsigned int & far_port )
{
...
{
	return G::Str::head( s , s.find_last_of(host_service_separators) ) ;
}
