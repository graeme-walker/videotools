...
/// 
class GNet::ServerPeer : public EventHandler , public Connection , public SocketProtocolSink
{
public:
	...
	virtual void onException( std::exception & ) g__final override ;
		///< Override from GNet::EventHandler. Calls doDelete(). Note 
		///< that the exception text is available via onDelete().
		///< (This is public so that a derived-class server containing
		///< something like a HeapClient can merge its exception 
		///< events.)
