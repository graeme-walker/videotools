...
#ifndef G_LOCATION__H
#define G_LOCATION__H
...
/// with update().
/// 
/// An extended host/service format is used for SOCKS support: before the 
/// "@" separator is the host/service pair passed verbatim to the socks 
/// server for it to resolve (see socksFarHost()); after the "@" is the 
/// host/service pair for the socks server itself, which we resolve and 
/// connect to (see host()).
/// 
/// \see GNet::ClientPtr
/// 
...
public:
	...
	explicit Location( const std::string & location_string , int family = AF_UNSPEC ) ;
		///< Constructor taking a formatted host/service string, also
		///< supporting an extended format for socks. Throws if an 
		///< invalid format.
