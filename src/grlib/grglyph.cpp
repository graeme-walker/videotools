...
// from a perl module
static unsigned int font_data [256][8] = {
...
unsigned int * Gr::Glyph::row( char c )
{
	size_t index = static_cast<size_t>(static_cast<unsigned char>(c)) ;
	G_ASSERT( index <= 0xffU ) ;
	index = index & 0xffU ;
	return font_data[index] ;
}

std::vector<std::vector<bool> > Gr::Glyph::matrix( char c )
{
	typedef std::vector<std::vector<bool> > matrix_t ;

	matrix_t matrix ;
	unsigned int * row_data = row( c ) ;
	for( int dy = 0 ; dy < 8 ; dy++ )
	{
		matrix.push_back( std::vector<bool>() ) ;
		unsigned int bits = row_data[dy] ;
		unsigned int mask = 256U ;
		for( int dx = 0 ; dx < 8 ; dx++ )
		{
			mask >>= 1 ;
			matrix.back().push_back( (bits & mask) != 0U ) ;
		}
	}
	return matrix ;
}

bool Gr::Glyph::point( char c , unsigned int x , unsigned int y )
{
	unsigned int bits = row(c)[y] ;
	unsigned int mask = 256U >> x ;
	return ( bits & mask ) != 0U ;
}
